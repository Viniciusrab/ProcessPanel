...
import { collection, addDoc, updateDoc, doc, query, orderBy, onSnapshot, Timestamp } from 'firebase/firestore';
...
export function useNotifications() {
    ...
    const initialLoadRef = useRef(true); // Controlar o carregamento inicial
    ...
    const tocarSomNotificacao = () => {
        const sound = new Howl({
            // src: ['https://www.soundjay.com/buttons/sounds/button-8.mp3'],
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(
            ...
            (snapshot) => {
                ...
                // Se for o carregamento inicial, apenas armazena os IDs sem mostrar notificações
                if (initialLoadRef.current) {
                    lastProcessedIdsRef.current = new Set(notificacoesData.map(n => n.id));
                ...
                // Identifica notificações que são realmente novas (criadas nos últimos 30 segundos)
                const agora = new Date();
                const novasNotificacoesParaAlerta = notificacoesData.filter(n => {
                    const dataCriacao = new Date(n.dataCriacao);
                    const diferencaEmSegundos = (agora.getTime() - dataCriacao.getTime()) / 1000;
                    return diferencaEmSegundos <= 30 && !lastProcessedIdsRef.current.has(n.id);
                });

                // Atualiza o ref com os novos IDs
                novasNotificacoesParaAlerta.forEach(n => {
                    lastProcessedIdsRef.current.add(n.id);
                });
